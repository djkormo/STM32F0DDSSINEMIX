...
void ClrBuf(void){
	uint32_t Cnt;
	//for(Cnt = 0; Cnt<GBufS; Cnt++) GBuf[Cnt] = 0;
	memset(GBuf, 0, GBufS*sizeof(uint8_t));
}
...
int PNum(int32_t Num, int16_t X, int16_t Y, int8_t Pad, uint8_t Size, PixT Inv){
	...
	for(Cnt = 0; Cnt<9; Cnt++) NBuf[Cnt] = 0;
	if(X<0) return -2;
...
void DrawCol(uint8_t Col, uint8_t Val){
	...
	for(Cnt = 0; Cnt<Val; Cnt++){
		WritePix(Col, YPix-Cnt-1, PixNorm);
	}
}

//13x7 pixels
uint8_t BatteryIcon(uint8_t X, uint8_t Y, uint8_t Val){
	Square(X, Y, X+11, Y+6, PixNorm);
	LineL(X+12, Y+2, X+12, Y+4, PixNorm);

	if(Val>0){
		LineL(X+2, Y+2, X+2, Y+4, PixNorm);
		LineL(X+3, Y+2, X+3, Y+4, PixNorm);
	}
	if(Val>1){
		LineL(X+5, Y+2, X+5, Y+4, PixNorm);
		LineL(X+6, Y+2, X+6, Y+4, PixNorm);
	}
	if(Val>2){
		LineL(X+8, Y+2, X+8, Y+4, PixNorm);
		LineL(X+9, Y+2, X+9, Y+4, PixNorm);
	}
	return X+13;
}
